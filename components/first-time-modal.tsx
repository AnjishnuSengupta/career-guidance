...
export function FirstTimeModal({ onClose }: FirstTimeModalProps) {
  ...
  const toggleVideoPreference = (pref: string) => {
    setSelectedVideoPrefs((prev) => (prev.includes(pref) ? prev.filter((p) => p !== pref) : [...prev, pref]))
  }
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-xl">
        ...
        <div className="py-4">
          ...
          <div className="flex items-center justify-between mb-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center">
                ...
                {i < 3 && <div className={`h-1 w-16 ${i < step ? "bg-purple-600" : "bg-gray-200"}`} />}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="space-y-4">
              <h3 className="text-lg font-medium">What subjects interest you?</h3>
              <p className="text-gray-600 mb-4">
                Select all that apply. This helps us recommend suitable career paths.
              </p>
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <p className="text-gray-600 mb-4">This helps us recommend nearby institutions and opportunities.</p>
          ...
          {step === 3 && (
            <div className="space-y-4">
              <h3 className="text-lg font-medium">What content would you like to see?</h3>
              <p className="text-gray-600 mb-4">Select the types of videos and resources that interest you most.</p>
              <div className="grid grid-cols-2 gap-3">
                {videoPreferences.map((pref) => (
                  <Button
                    key={pref}
                    variant={selectedVideoPrefs.includes(pref) ? "default" : "outline"}
                    className={`justify-start ${
        ...
        <DialogFooter className="flex flex-col sm:flex-row sm:justify-between sm:space-x-2">
          {step > 1 && (
            <Button variant="outline" onClick={handleBack} className="mb-2 sm:mb-0">
              Back
            </Button>
          )}
